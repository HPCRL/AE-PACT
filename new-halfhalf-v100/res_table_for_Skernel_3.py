...
def add_features(K, H, C, S, Stride, Th, Tw, Tk, Wh, Ww, Wk, Bh, Bw, Bk, alt, machine_info):
    ...
    # if alt == 2:
    #     return [K, H, C, S, Stride, Th, Tw, Tk, Wh, Ww, Wk, Bh, Bw, Bk, Ops, G2S_trans, S2R_trans, R2S_trans, S2G_trans,recp_occupancy, correct_wave, conc_tb]
    if alt == 2:
...
def expand_csv(file, alt, stride, if_train, if_norm, pass_norm_factor, machine_info):
    ...
    #     new_header = "K, H, C, S, Stride, Th, Tw, Tk, Wh, Ww, Wk, Bh, Bw, Bk, Ops, G2S_trans, S2R_trans, S2G_trans, R2S_trans, recp_occupancy, correct_wave, conc_tb, Time"
    elif alt == 2:  # problem sizes, stride, tile sizes, OPs, GM-transactions, SM-transactions,
...
def S3(dirpath, machine_info,  topN, step, initN, all_model_dict, algo_list, algo_name):
    ...
    # summary_file = open("s_3_summary.csv", "w")
