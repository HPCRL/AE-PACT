...
def calcAll(argv):
    ...
    rs = int(argv[4]) #3
    rsADJ = rs
    if rs == 1: # TODO: Will this fix the rs = 1 case?
        rsADJ = 2
    layer_name = argv[5]
    path = os.getcwd()
    ...
    OptimalInputElementsMoved = (H*2+rs-2)*(W*2+rs-2)*(C) + 0.0
                        ...
                        for T_h in range(1,32):
                            for T_w in range(1,32):
                                for T_c in range(1,2): # Limited for now
                                    ...
                                    # CHECK the total number of registers this block will use
                                    registersUsed = T_h * T_w * T_k + (T_h * 2 + rsADJ - 2) * (T_w * 2 + rsADJ - 2) * T_c + 1
                                    ...
                                    # CHECK Shared Memory ----------
                                    sharedMemoryIn = (B_h * T_h * 2 + rsADJ - 2) * (B_w * T_w * 2 + rsADJ - 2) * (W_c * T_c)
                                    ...
                                    if t == 0:
                                        numberOfElementsPerBlockMovedToSM = (C/mult) * (B_h * T_h * 2 + rsADJ - 2) * (B_w * T_w * 2 + rsADJ - 2)
                                    ...
                                    gen_files(H, W, K, C, rs, rs, T_k, T_h, T_w, 32, B_h, B_w, mult, registersUsed+2)
                                    create_cmake(H, W, K, C, rs, rs, T_k, T_h, T_w, 32, B_h, B_w, mult, registersUsed+2)
                                    ...
                                    doLaunch(path + "/build", path + "/build/"+layer_name+"/timesForProblem"+filename+".txt", filename, registersUsed+2)
                                    counter += 1
...
def gen_files(H, W, K, C, R, S, Tk, Th, Tw, Bk, Bh, Bw, CMult, RegLim):
    ...
    fCSG = open("template_st2_3.code", "r")
    for line in fCSG:
        strpLine = line
        if oneM in strpLine:
            newPGM += one + "\n"
...
def create_cmake(H, W, K, C, R, S, Tk, Th, Tw, Bk, Bh, Bw, CMult, RegLim):
    ...
    problemNew = "\nset(CUDAPROBLEMSIZE \"-DOT=1 -DN_R={} -DR={} -DN_S={} -DS={} -DN_B=1 -DN_F={} -DK={} -DN_C={} -DC={} -DN_W={} -DW={} -DN_H={} -DH={} -DStrideW=2 -DStrideH=2 -DPaddingH=0 -DPaddingW=0\")"
