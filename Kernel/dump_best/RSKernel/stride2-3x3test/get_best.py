            ...
            if layer[0] in kernels:
                ...
                if problem not in mapping: ignore = True
                    ...
                    if kernel == "RSkernel":
                        ...
                        rsADJ = R
                        if R == 1: # TODO: Will this fix the rs = 1 case?
                            rsADJ = 2
                        registersUsed = TH * TW * TK + (TH * 2 + rsADJ - 2) * (TW * 2 + rsADJ - 2) + 1

                        reproduce_code_RSker(H, W, K, C, R, R, TK, TH, TW, BK, BH, BW, CMult, registersUsed+2, mapping_name)
